...
/* Note common use of NMC2/GOES_WEST type */
...
#define DEFAULT_RETRANS_ENTRIES_NMC 200000
#define DEFAULT_RETRANS_ENTRIES_NMC2 100000
#define DEFAULT_RETRANS_ENTRIES_NOAAPORT_OPT 50000
#define DEFAULT_RETRANS_ENTRIES_NMC3 50000
#define DEFAULT_RETRANS_ENTRIES_NMC1 50000
...
#define	MAX_SBN_TYP           8	/* MAX type for now	*/	
...
typedef struct prod_retrans_entry {
	...
	unsigned short  prod_run_id;		/* 2 bytes NCF run_id */
	unsigned short  prod_orig_run_id;	/* 2 bytes NCF run_id */
	char    prod_type;			/* 1 byte (GOES_EAST, GOES_WEST, NWSTG, */
								/*          NOAAPORT_OPT, etc) */
	char    prod_cat;			/* 1 byte (TEXT, GRAPHIC, IMAGE, GRID, */
								/*          POINT, IMAGE) */
	unsigned short  prod_code;			/* 2 bytes (code 0,1,2,...,n) */
	unsigned short  prod_sub_code;		/* 2 bytes (subcode 0,1,2,...,n) */
	unsigned short  prod_status;		/* 2 bytes */
...
typedef struct entry_info {
	...
	unsigned short  run_id_last;                /* last run_id for this type */
	unsigned short  run_id_orig_last;           /* last run_id for this type */
	unsigned long   tot_prods_rcvd;             /* all prods rcvd by proc */
	unsigned long   tot_prods_lost;             /* tot prods lost any reason */
	unsigned long   tot_prods_lost_seqno;       /* tot prods lost seqno proc only */
	unsigned long   tot_prods_lost_abort;       /* prods lost abort (proc or client) */
	unsigned long   tot_prods_lost_other;       /* prods lost other (proc or client) */
	unsigned long   tot_prods_retrans_rcvd;     /* prods retrans rcvd by proc */
	unsigned long   tot_prods_retrans_rcvd_lost; /* prods retrans rcvd lost */
	unsigned long   tot_prods_retrans_rcvd_notlost; /* prods rcvd not lost */
	unsigned long   tot_prods_retrans_rqstd;    /* prods retrans requested */
...
typedef  struct pipe_retransmit_hdr  {   
	unsigned long	pipe_request_numb;		/* seq numb of request */
	...
	long	pipe_cpio_addr;			/* cpio address */
	long	pipe_request_time;		/* time of request */
	long	pipe_first_prod_seqno;	/* first missing prod_seqno */
	long	pipe_last_prod_seqno;	/* last missing prod_seqno */
	long	pipe_run_numb;			/* unique run numb */
	unsigned short	pipe_delay_send;		/* time to delay for each send to NCF */
...
typedef struct acq_table {
	...
	unsigned short	proc_blkno;					/* block number */
	unsigned long	proc_prod_seqno;			/* Current seqno */
	unsigned long	proc_base_prod_seqno_last;	/* prod seqno last processed */ //(updated from curr_prod_seqno)
	unsigned long	proc_orig_prod_seqno_last;	/* last orig prod_seqno retrans */ //(Updated from psh->seqno)
	unsigned long	proc_prod_bytes_read;
	unsigned short	proc_prod_run_id;    /* Unique run identification    */// (psh->runid)
							   /*    for product stream    */
							   /*    parm for retransmission   */
	unsigned short	proc_orig_prod_run_id;    /* Unique run identification    */ //(Update from psh->orig_runid)
	...
	/* Runtime statistics */
	unsigned long	proc_tot_prods_lost_errs; /* total prod lost errors */
	unsigned long	read_tot_buff_read;		/* total bufs read */
	unsigned long	read_frame_tot_lost_errs; /* read frame total lost errors */
	unsigned long	proc_tot_prods_handled;	/* total products for process */
	unsigned long	proc_tot_prods_retrans_rcvd; /* prods retrans rcvd by proc */
	unsigned long	proc_tot_prods_retrans_rcvd_lost; /* prods retrans rcvd lost */
	unsigned long	proc_tot_prods_retrans_rcvd_notlost; /* prods rcvd not lost */
	unsigned long	proc_tot_prods_retrans_rqstd; /* prods retrans request by proc */
	unsigned long	proc_acqtab_prodseq_errs;	/* prodseq errs */
...
typedef  struct	buff_hdr  {		/* buffer headers for each prod hdr */
	...
	unsigned short	proc_blkno; 		/* process assgn put blkno */
	char	proc_prod_type;		/* prod type */
	char	proc_prod_cat;		/* prod category */
	unsigned short	proc_prod_code;		/* prod code */
	unsigned short	proc_sub_code;		/* prod subcode */
	unsigned short	proc_prod_scale;	/* prod scale, */
	...
	unsigned short	buff_tot_length;	/* tot length of buff ref by buff_ptr */
	unsigned short	buff_data_length;	/* length of prod data in buff */

	unsigned short	buff_data_offset;	/* begin offset of prod data in buff */
					/*   within buff data_length */
	unsigned short	buff_bytes_record;	/* bytes per record in buffer */
					/*    should be same for all buffs */
					/*    of individual product */
	unsigned char   buff_records_per_blk;  /* Numb of record per blk     */
					/* i.e. multiple records in blk*/
	unsigned char   buff_blks_per_record;  /* Numb of blks per record    */
					/* i.e., record spans numb blks */
		/* Optional header lengths, if nonzero */
	unsigned short	buff_commhdr_length;	/* length of comm hdr in buff */
					/*   e.g. SBN protocol header */
	unsigned short	buff_xfrhdr_length;	/* length of xfr hdr in buff */
					/*   e.g. AWIPS prod xfr header in SBN*/
	unsigned short	buff_datahdr_length;	/* length of data hdr in buff */
					/*   e.g. NESDIS, WMO, etc */
...
unsigned long   total_prods_retrans_rcvd;     /* prods retrans rcvd by proc */
unsigned long   total_prods_retrans_rcvd_lost; /* prods retrans rcvd lost */
unsigned long   total_prods_retrans_rcvd_notlost; /* prods rcvd not lost */
unsigned long   total_prods_retrans_rqstd;    /* prods retrans requested */
unsigned long   total_prods_handled;    /* prods retrans requested */
unsigned long   total_prods_lost_err;
unsigned long   total_frame_cnt;
unsigned long   total_frame_err;
