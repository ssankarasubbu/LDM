 ...
 *   This routine is used to initialize the retransmission table setup
 *   on the heap .
 *
 *   @retval -1    Error 
 *   @retval  0    Success 
...
int init_retrans(PROD_RETRANS_TABLE **pp_prod_retrans_table)
{
        ...
        if (pl_prod_retrans_table != NULL){
                ...
                /*ii = GET_RETRANS_TABLE_TYP(sbn_type);*/
                        ...
                        for(iii=0;
                                ...
                                p_retrans_entry[iii].prod_arrive_time = 0;
                                p_retrans_entry[iii].prod_seqno = 0;
                ...
                if(ulogIsDebug()){
                        udebug("%s  OK init retrans_table for channel [%s]\n",FNAME);
                        udebug("              numb_entries     (%d)\n",
                        pl_prod_retrans_table->entry_info[ii].numb_entries);
                }
        ...
        if(ulogIsDebug())
                udebug("%s Exiting  init retrans_table   base=0x%x\n", FNAME, pl_prod_retrans_table);
        return(0);

} /* end init_retrans */

/**
 *   init_acq_table -  Routine to initialize acquisition table
 *   This routine is used to initialize the acquisition table setup
...
/**
 *   do_prod_lost -  Routine to handle lost products 
 *   This routine is used to handle lost products and initiates retransmission
 *   request that is sent to A/BNCF.
 *
 *   @retval -1    Error 
 *   @retval  0    Success 
 *  
...
/**
 *   generate_retrans_rqst -  Routine to generate retransmission requests
 *   This routine is used to generate retransmission requests for the lost 
 *   product(s) received via SBN.
 *
 *   @retval -1    Error 
 *   @retval  0    Success 
 *  
                ...
                int rqst_cause)
{
        ...
        p_pipe_requesthdr->pipe_first_prod_seqno = first_prod_seqno;
        p_pipe_requesthdr->pipe_last_prod_seqno = last_prod_seqno;
        p_pipe_requesthdr->pipe_run_numb = (long) p_acqtable->proc_prod_run_id;
        p_pipe_requesthdr->pipe_cpio_addr = global_cpio_addr;
        time(&p_pipe_requesthdr->pipe_request_time);
        ...
        /* Now write to pipe */
        if(write(global_retransmitpipe_fd,
